...
def extract_text_with_layout(filename: str, file_bytes: bytes) -> tuple[str, dict]:
    ...
    if filename_lower.endswith(".pdf"):
        return _extract_pdf_with_layout(file_bytes)
...
def _extract_pdf_with_layout(file_bytes: bytes) -> tuple[str, dict]:
    text_parts: list[str] = []
    pages = 0
    image_blocks = 0
    text_blocks = 0
    ...
    with fitz.open(stream=file_bytes, filetype="pdf") as doc:
        ...
        for page in doc:
            ...
            for block in blocks:
                ...
                # Heuristic: parallel spans per line → table-like
                if len(lines) >= 4:
                ...
                if lines:
                    ...
                    bot_txt = " ".join(
                        span.get("text", "") for span in lines[-1].get("spans", [])
                    ).strip()
    ...
    header_footer_repeats = sum(
        1 for _, c in {**top_lines, **bottom_lines}.items() if c >= 2
    )
